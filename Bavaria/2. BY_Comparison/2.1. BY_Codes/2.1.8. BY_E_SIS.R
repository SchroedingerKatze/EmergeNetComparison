...
BY_Trans_Pr<-list()
for (i in 1:7) {
  BY_Trans_Pr[[i]]<-as.matrix(read.table(paste0("E:/Work/Bavaria/2. BY_Comparison/2.2. BY_Data/2.2.6. BY_Transfer_Probability Data/BY_Trans_Pr", i%%7, ".txt")))
}
names(BY_Trans_Pr)<-c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat")
...
BY_WD_N<-as.matrix(read.table("BY_WD_N.txt", header = T))

#2# Function for epidemic simulation
## Initial parameter settings
#### Initial proportion of infectious patients in each node
Prop_start<-rep(c(0.005, 0.0005), each = dim(BY_WD_N)[1]/2)
...
Fun_epi_d<-function(I,N,d){
  ...
  ###### N on day d
  N_d<-as.numeric(BY_WD_N[,WD])
  ###### I on day d
  I_d<-c(Fun_epi_I(T_rate_H,R_rate,N_H,I_H),Fun_epi_I(T_rate_C,R_rate,N_C,I_C))%*%BY_Trans_Pr[[WD_minus]]
  ###### S on day d
  S_d<-N_d-I_d
  #### Results
...
for (n_T in seq(0.5,5,0.5)) {
  ...
  write.table(INS[[1]],paste0("BY_I_", n_T, "_", n_R, "_", n_C,".txt"))
  write.table(INS[[2]],paste0("BY_N_", n_T, "_", n_R, "_", n_C,".txt"))
  write.table(INS[[3]],paste0("BY_S_", n_T, "_", n_R, "_", n_C,".txt"))
}

## Varying n_R
for (n_R in seq(0,5,0.5)) {
  #### Parameters
  n_T<-1
  n_C<-0
  T_rate_H<-n_T*T_rate_0
...
I<-as.matrix(read.table("BY_I_1_1_0.txt"))
N<-as.matrix(read.table("BY_N_1_1_0.txt"))
plot(colSums(I[1:357,])/colSums(N[1:357,]))
plot(colSums(I[357:714,])/colSums(N[357:714,]))
