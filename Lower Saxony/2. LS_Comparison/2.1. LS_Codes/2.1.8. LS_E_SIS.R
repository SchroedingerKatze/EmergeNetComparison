...
LS_Trans_Pr<-list()
for (i in 1:7) {
  LS_Trans_Pr[[i]]<-as.matrix(read.table(paste0("E:/Work/Lower Saxony/2. LS_Comparison/2.2. LS_Data/2.2.6. LS_Transfer_Probability Data/LS_Trans_Pr", i%%7, ".txt")))
}
names(LS_Trans_Pr)<-c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat")
...
LS_WD_N<-as.matrix(read.table("LS_WD_N.txt"))
colnames(LS_WD_N)<-LS_WD_N[1,]
LS_WD_N<-LS_WD_N[-1,]

#2# Function for epidemic simulation
## Initial parameter settings
#### Initial proportion of infectious patients in each node
Prop_start<-rep(c(0.005, 0.0005), each = dim(LS_WD_N)[1]/2)
...
Fun_epi_d<-function(I,N,d){
  ...
  ###### N on day d
  N_d<-as.numeric(LS_WD_N[,WD])
  ###### I on day d
  I_d<-c(Fun_epi_I(T_rate_H,R_rate,N_H,I_H),Fun_epi_I(T_rate_C,R_rate,N_C,I_C))%*%LS_Trans_Pr[[WD_minus]]
  ###### S on day d
  S_d<-N_d-I_d
  #### Results
...
for (n_T in seq(0.5,5,0.5)) {
  ...
  write.table(INS[[1]],paste0("LS_I_", n_T, "_", n_R, "_", n_C,".txt"))
  write.table(INS[[2]],paste0("LS_N_", n_T, "_", n_R, "_", n_C,".txt"))
  write.table(INS[[3]],paste0("LS_S_", n_T, "_", n_R, "_", n_C,".txt"))
}
...
I<-as.matrix(read.table("LS_I_5_1_0.txt"))
N<-as.matrix(read.table("LS_N_5_1_0.txt"))
plot(colSums(I[1:211,])/colSums(N[1:211,]))
